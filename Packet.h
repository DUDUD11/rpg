...
{
	INT8 TARGET_ANIMATION_X;
	INT8 TARGET_ANIMATION_Y;
};
...
struct OBJECT_ATTACK_REQUEST : public PACKET_HEADER
{
	char ATTACK_CHARACTER_ID[MAX_CHARACTER_ID_LEN + 1];
	char DAMAGED_CHARACTER_ID[MAX_CHARACTER_ID_LEN + 1];
};

const UINT32 OBJECT_ATTACK_REQUEST_LENGTH = sizeof(OBJECT_ATTACK_REQUEST);

struct OBJECT_ATTACK_BROADCAST_PACKET : public OBJECT_ATTACK_REQUEST
{
	UINT32 DAMAGED_HP;
};
